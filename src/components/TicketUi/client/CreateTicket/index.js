...
import { ClientvalidateForm } from "../../../../utils/FormValidator";
...
export default function CreateTicketForm({ handleCloseDetail, showNotification, prefilledData }) {
    ...
    const { addTicket ,USERNAME_LIST } = useTicket();
    ...
    const handleSubmit = () => {
        ...
        if (Object.keys(newErrors).length === 0) {
            const ticketWithId = {
                ...form,
                createdby: user?.name,
                projectCode : CompanyInfo?.companyId ,
                appname :2,
            ...
            console.log("🚀 ~ handleSubmit ~ ticketWithId:", ticketWithId)
            if (prefilledData) {
