  ...
  const validateForm = () => {
    ...
    if (!assignments || assignments.length === 0) {
      ...
      assignments.forEach((item, index) => {
        const hrs = Number(item.estimate?.hours ?? item.EstimatedHours);
        if (!item.department || !item.user || isNaN(hrs) || hrs <= 0) {
          newErrors[`assignments_${index}`] = "Department, user, and estimated hours (> 0) are required for all assignments.";
        }
      });
    }
