...
const DeleteTrainingAction = ({ params, showNotification }) => {
	...
	return (
		<>
			<IconButton size="small" color="error" sx={{ textTransform: "none" }} title="Delete Training Session" onClick={() => setShowDeleteModal(true)}>
				<BadgeX />
			</IconButton>
			<ReusableConfirmModal open={showDeleteModal} onClose={() => setShowDeleteModal(false)} onConfirm={handleDelete} type="delete" />
		</>
	);
};
