...
const Dashboard = ({ Data, filters }) => {
	...
	// Training Type
	React.useEffect(() => {
		const availableTypes = Object.keys(trainingTypesDistribution).filter((key) => trainingTypesDistribution[key] > 0);
	...
	// Training Mode
	React.useEffect(() => {
		const availableModes = Object.keys(trainingModeDistribution).filter((key) => trainingModeDistribution[key] > 0);
