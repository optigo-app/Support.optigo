...
const TrainingActionButton = ({ params, onSchedule, isClient }) => {
	...
	const isAlreadyScheduled = params?.row?.training?.[0]?.schedule?.dateTime != null && params?.row?.training?.[0]?.schedule?.zoomLink;
	...
	const handleSchedule = () => {
		onSchedule(params?.row?.ticketNo, "training", {
			dateTime: dateValue,
			zoomLink,
		});
		setDateValue(null);
	...
	return (
		<>
			{isAlreadyScheduled ? (
				...
				<>
					<Tooltip title={isClient ? "You can't schedule a training for this ticket. Please contact the Team." : ""}>
						{" "}
						<Chip
					...
					>
						<TeamMemberCard member={params.row} isScheduled={false} dateValue={dateValue} onDateChange={setDateValue} zoomLink={zoomLink} onZoomLinkChange={(e) => setZoomLink(e.target.value)} onSchedule={handleSchedule} />
					</Popover>
				</>
			)}
		</>
					...
					<Grid container spacing={2}>
						...
						<Grid item xs={6}>
							...
							{data?.schedule?.zoomLink ? (
								<Typography component="a" href={data.schedule.zoomLink} target="_blank" rel="noopener noreferrer" variant="body2" sx={{ display: "block" }}>
									{data.schedule.zoomLink}
								</Typography>
							) : (
