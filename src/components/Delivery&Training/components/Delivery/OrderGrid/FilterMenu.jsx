...
const serviceTypes = ["Free", "Complementary", "Chargeable"];
...
export function FilterMenu({ showAdvanceFilter = false, open = false, setOpen, filters, setFilters, clearFilters }) {
            ...
            <Box sx={{ mb: 4 }}>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                {onDemandOptions.map((option) => (
                  <FilterChip
                    key={option}
                    label={option === "yes" ? "Client" : "Optigo"}
            ...
            {showAdvanceFilter && (
              <Box sx={{ mb: 4 }}>
                ...
                >
                  ...
                  <RotatingIcon filterKey="currentStatus" onReset={handleFilterReset} />
                </Box>
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                  {statusList?.map((status) => (
                    <FilterChip
                      key={status?.label}
                      label={status?.label}
                      selected={filters?.currentStatus?.includes(status?.label)}
                      onClick={() => {
                        const newcurrentStatus = filters?.currentStatus?.includes(status?.label) ? filters?.currentStatus?.filter((m) => m !== status?.label) : [...filters?.currentStatus, status?.label];
                        updateFilters({ currentStatus: newcurrentStatus });
                      }}
                    />
                  ))}
                </Box>
