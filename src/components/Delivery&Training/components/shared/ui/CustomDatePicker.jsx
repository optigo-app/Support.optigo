...
import { TextField, Box, Popover, InputAdornment, Button, Stack, IconButton, ThemeProvider, Chip } from "@mui/material";
...
const CustomDualDatePicker = ({ showAdvanceFilter = false, value = {}, onChange = () => {} }) => {
  ...
  const Moredatefields = [
    { label: "Approve Date", value: "Approve Date" },
    // { label: "Sample Approve Date", value: "Sample Approve Date" },
  ];

  const DateFiltersOptiosn = showAdvanceFilter ? [...DATE_FIELDS, ...Moredatefields]?.filter(({ label }) => label !== "Delivery Date") : DATE_FIELDS;
    ...
    <ThemeProvider theme={Datetheme}>
      <Box display="flex" alignItems="center" sx={{ zIndex: 999 }}>
        <TextField
          ...
          placeholder="🗓️Select Date Range"
          readOnly
          InputProps={{
            startAdornment: (
              <>
                ...
                />
                <MenuList options={DateFiltersOptiosn} anchorEl={anchorElStatus} open={Boolean(anchorElStatus)} handleClose={() => setAnchorElStatus(null)} handleSelect={handleStatusSelect} selectedValue={tempRange?.status || "All"} />
              </>
            ),
