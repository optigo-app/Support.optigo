...
const AutocompleteComponent = ({ label, options, value, isWantLabel = false, onChange, disabled, helperText = "", error = false, ...props }) => {
	const findOptionByValueOrLabel = (val) => {
		return options.find((option) => option.value === val || option.label === val);
	};
	...
	return (
		<Autocomplete
			...
			filterOptions={(options, { inputValue }) => {
				...
				return options.filter((option) => option.label?.toLowerCase().includes(inputValue.toLowerCase()) || option.value?.toString().toLowerCase().includes(inputValue.toLowerCase()));
			}}
