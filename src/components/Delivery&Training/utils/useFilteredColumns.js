import { getPointGridColumns } from "../../PointToBeDiscuss/components/PointGrid/ColumnsProps";
...
export const useFilteredColumns = ({ role, isClient, callbacks }) => {
  ...
  return allColumns.filter((col) => allowedFields.includes(col.field)).map((col) => ({ ...col, ...(role === "client" ? { flex: 1 } : {}) }));
};




export const usePointGridColumns = ({ role, isClient, callbacks }) => {
  const columns = getPointGridColumns(...callbacks);
