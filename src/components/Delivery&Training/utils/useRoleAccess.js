export const useRoleAccess = (user) => {
	...
	const roleId = typeof rawRoleId === "boolean" ? (rawRoleId ? 1 : 0) : Number(rawRoleId) || 0;

	const rawRole = user?.role;
	const roleName = typeof rawRole === "string" ? rawRole.toLowerCase() : String(rawRole || "").toLowerCase();

	const isRole = (targetRole, targetId) => roleName === targetRole || roleId === targetId || String(roleId) === String(targetId);
