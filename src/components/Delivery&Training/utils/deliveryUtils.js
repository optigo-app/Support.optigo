...
export const filterDeliveryData = (tickets, filters = null) => {
	...
	const { search = "", approval = "", projectCode = null, topicType = "", serviceType = [], onDemandOption = "", paymentMethod = [], paymentStatus = [], isFavorite = false, date = {}, Tabs = -1, deliveryStatus = "" } = filters;

	const hasActiveFilters = Boolean(
		search || approval || projectCode || topicType || (serviceType && serviceType.length > 0) || onDemandOption || (paymentMethod && paymentMethod.length > 0) || (paymentStatus && paymentStatus.length > 0) || isFavorite || date?.startDate || date?.endDate || Tabs !== -1 || deliveryStatus,
	);
		...
		if (!dateStatus) {
			...
			switch (normalizedStatus) {
				...
				case "created_date":
					...
					break;
				default:
					const statusKey = Object.keys(ticket).find((key) => key.toLowerCase().includes(normalizedStatus) || normalizedStatus.includes(key.toLowerCase()));
	...
	return tickets?.filter((ticket) => {
		try {
			...
			return matchesSearchQuery && matchesApproval && matchesProjectCode && matchesTopicTypeFilter && matchesServiceTypeFilter && matchesOnDemand && matchesPaymentMethodFilter && matchesPaymentStatusFilter && matchesFavorite && matchesDateRange && matchesTab && matchesDeliveryStatus;
		} catch (error) {
...
export const isAnyFilterActive = (filters) => {
	...
	return hasSearch || hasApproval || hasProjectCode || hasTopicType || hasServiceType || hasOnDemandOption || hasPaymentMethod || hasPaymentStatus || hasFavorite || hasDateFilter || hasTabs || hasDelivery;
};
