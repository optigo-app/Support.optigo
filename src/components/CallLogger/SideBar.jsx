      ...
      <Drawer anchor="left" open={open} onClose={onClose}>
        <Box sx={{ width: 500, p: 2, display: "flex", flexDirection: "column", height: "100vh" }}>
          <Box sx={{ flexGrow: 1 }}>
            ...
            {/* Company Selection */}
            <Autocomplete key={'company-input'} ref={companyInputRef} fullWidth options={companyOptions || []} value={selectedCompany} onChange={handleCompanyChange} getOptionLabel={(option) => option?.label || ""} isOptionEqualToValue={(option, value) => option?.value === value?.value} renderInput={(params) => <TextField {...params} label="Company Name" margin="normal" error={!!errors.company} helperText={errors.company} disabled={!!data} autoFocus />} />
            ...
            {/* Description */}
            <TextField fullWidth label="Description" value={formData?.description || ""} onChange={handleTextFieldChange("description")} margin="normal" error={!!errors.description} helperText={errors.description} multiline rows={2} />
