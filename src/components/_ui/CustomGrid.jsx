...
const DataGridDemo = () => {
	...
	const generateRows = useCallback((count) => {
		...
		const permissionSets = [["User Management", "Content Management", "Analytics"], ["Content Management", "Analytics"], ["Analytics"]];
	...
	const columns = [
		...
		},
		{
			...
			headerName: "Role",
			width: 130,
			renderCell: ({ value }) => <span style={{ fontStyle: value === "Admin" ? "italic" : "normal" }}>{value}</span>,
		},
		{
			field: "accessLevel",
		...
		},
		{
			...
			width: 130,
			renderCell: ({ value }) => <span style={{ fontWeight: value === "Admin" ? "bold" : "normal" }}>{value}</span>,
		},
		{
			field: "permissions",
			...
			{demoType === "client" ? (
				...
				<Box sx={{ height: "calc(100vh - 180px)", width: "100%" }}>
					<EnhancedDataGrid
						...
						defaultSortDirection="asc"
						pageSize={1000}
						pageSizeOptions={[10, 25, 50, 100, 1000, 10000, 1000000]}
						serverSidePagination={true}
						onServerPageChange={handleServerPageChange}
						totalRowCount={serverData.totalRowCount}
						loadingRows={serverData.loading}
					/>
				</Box>
			)}
		</Container>
