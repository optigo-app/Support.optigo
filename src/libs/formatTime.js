import { format, intervalToDuration, parse ,parseISO, differenceInSeconds, formatDistanceToNow, isValid } from "date-fns";
import TimeIcon from "@mui/icons-material/AccessTime"; // Clock icon
import { Event as EventIcon, Call as CallIcon, Description as DescriptionIcon, Info as InfoIcon, Forward as ForwardIcon, CheckCircle as CheckCircleIcon, Comment as CommentIcon, PriorityHigh as PriorityHighIcon, Star as StarIcon, Feedback as FeedbackIcon, SentimentSatisfiedAlt as SentimentSatisfiedAltIcon, ConfirmationNumber as ConfirmationNumberIcon, Timer as TimerIcon, CallEnd as CallEndIcon } from "@mui/icons-material";
...
export const formatDuration = (callStart, callClosed) => {
  ...
  return [duration.hours ? `${duration.hours} hr` : "", duration.minutes ? `${duration.minutes} min` : "", duration.seconds ? `${duration.seconds} sec` : ""].filter(Boolean).join(" ");
};
...
export const generateSummary = (callData) => {
  if (!callData)
    return {
      details: "No call details available.",
  ...
  const details = callDetails || `This call was about "${topic}". It lasted for ${callDuration} seconds and was categorized as "${priority}" priority.`;
